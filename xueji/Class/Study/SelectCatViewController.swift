...
import UIKit
typealias SelectCatViewControllerBlock = (_ model : CategoryListModel_list)->()
let SelectedCatCellID = "SelectedCatCell_id"

class SelectCatViewController: BaseViewController,UITableViewDelegate,UITableViewDataSource,CategoryListApiMangerViewControllerDelegate {
...
//        self.view.addSubview(compleBtn)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (selectedCatBlock != nil) {
            self.selectedCatBlock(requestManger.getModel(index: indexPath.row))
        }
